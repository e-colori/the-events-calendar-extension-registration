...
class Event_Registration_Public {
  ...
  /**
    ...
    public function wpecr_registration_button( $atts ) {
		...
		echo $dateStart	= get_gmt_from_date($event_date);
		...
		$data = 
			'<form action="' .$this->registration_button_url(). '" method="post" class="wpecr_registration">
				...
				<input type="hidden" name="wpecr[olddate]" value="'. $event_date .'">
				<input type="hidden" name="wpecr[newdate]" value="'. $dateStart .'">
				<input type="hidden" name="wpecr[date]" value="'. $event_date_new .'">
    ...
    public function wpecr_registration( $atts ) {
    	...
    	if( isset( $_POST['action'] ) && $_POST['action'] =='order_send' ) {
				...
				if ( $page_message= $this->wpecr_registration_send() ) {					
					...
					$data .='<a href=" '. home_url() .'">'. __( 'Return to Home', 'wpecr' ) .'</a>';					
				}else{
					//NO email
    ...
    public function wpecr_registration_send() {
		...
		$message = $page_message = sprintf( __( 'Thank you %1$s, <br/><br/> your registration was successful.', 'wpecr' ), $to );
